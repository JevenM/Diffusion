...
is_iid=2
num_users = 10 # 10
# data_dir = '/data/mwj/mycode1/FGL_FrameWork/data'
data_dir = './data'
use_dataset = 'SVHN' #'Mnist' #'SVHN' #'Cifar10' #'stl10' 
inChannel=3
verbose = True
local_train_bs = 256
test_bs = 256
...
if use_dataset == 'Cifar10':
    ...
    trans_cifar10_test = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.47375134, 0.47303376, 0.42989072], std=[0.25467148, 0.25240466, 0.26900575])])
    ...
    trans_cifar10_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.48836562, 0.48134598, 0.4451678], std=[0.24833508, 0.24547848, 0.26617324])])

    # 下载数据集
    train_dataset = datasets.CIFAR10(root=data_dir, train=True, transform=trans_cifar10_train, download=False)
    test_dataset = datasets.CIFAR10(root=data_dir, train=False, transform=trans_cifar10_test, download=False)
    ...
    trans_mnist_test = transforms.Compose([
        ...
        transforms.Normalize((0.1307,), (0.3081,))])
    # 下载数据集
    train_dataset = datasets.MNIST(root=data_dir, train=True, transform=trans_mnist_train, download=True)
    test_dataset = datasets.MNIST(root=data_dir, train=False, transform=trans_mnist_test, download=True)
    ...
    train_dataset = datasets.SVHN(root=data_dir, download=True, split='train',
                                         transform=trans_svhn)

    test_dataset = datasets.SVHN(root=data_dir, download=True,
                                         transform=trans_svhn, split='test')
...
test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=test_bs, shuffle=False, num_workers=8, drop_last=False, pin_memory=True)

from torch.utils.data import random_split
train_d_len = len(train_dataset)
generator = torch.Generator().manual_seed(seed)
dataset_train, dataset_valid = random_split(train_dataset, [int(train_ratio*train_d_len), train_d_len - int(train_ratio*train_d_len)], generator)
...
log.info('Diffusion model total params: %.2fM' % (sum(p.numel() for p in global_model.parameters())/1000000.0))
# 打印模型的参数列表
for name, param in global_model.named_parameters():
    log.info(f"global model parameter name: {name}, Shape: {param.shape}, grad: {param.requires_grad}")
...
def demo(model, c_id, gen_img_num=100, g_name="None", in_channel=3):
    ...
    fakes_classes = torch.arange(num_classes, device=device).repeat_interleave(num_of_each_class, 0)
    ...
    # 存成整体图片
    utils.save_image(fakes, whole_img_dir+f'/whole_{gen_img_num}.png', nrow=num_of_each_class, normalize=True, scale_each=True)
    # 存成单个图像
    for i, img in enumerate(fakes):
        class_dir = whole_img_dir+"/"+str(i//num_of_each_class)
        os.makedirs(class_dir, exist_ok=True)
        # print(img.shape)
        utils.save_image(img, class_dir+f'/demo_{i%num_of_each_class}.png', nrow=1, normalize=True, scale_each=True)


# 定义客户端本地更新训练的类
class LocalUpdate(object):
    def __init__(self, id, dataset=None, device=device, idxs=None, local_model=None, local_bs=local_train_bs, local_epochs=local_epoch):
        '''
        Args:
        ---
        ...
        self.id = id
        self.trainloader = DataLoader(DatasetSplit(dataset, idxs), batch_size=self.local_bs, shuffle=True, pin_memory=True, drop_last=False
    ...
    def train(self):
        ...
        for iter in range(self.local_epochs):
            ...
            for batch_idx, (images, labels) in enumerate(self.trainloader):
                ...
                ema_update(self.local_model, self.model_ema, 0.95 if iter < 20 else ema_decay)
                self.scaler.update()

                if verbose and batch_idx % 10 == 0:
                    log.info('Update Epoch-train: {} [{}/{} ({:.0f}%)] Loss: {:.6f}\n'.format(
        ...
        return self.local_model.model.state_dict(), self.model_ema.model.state_dict(), sum(train_epoch_loss1) / len(train_epoch_loss1)
...
def test_inference_lt_prompt():
    ...
    for idx in range(num_users):
        ...
        log.info('test_inference_lt_prompt：| Client: {} | Local Test Loss: {:.5f}'.format(idx, total_loss / val_total))
...
def inference_client(model_ema, idx):
    ...
    with torch.no_grad():
        ...
        log.info('inference_client: | Client: {} | Local Test Loss: {:.5f}'.format(idx, mean_loss))
...
testloader_list = []
for idx in range(num_users):
    testloader_list.append(DataLoader(DatasetSplit(
        dataset_valid, dict_users_lt[idx]), batch_size=test_bs, shuffle=False, drop_last=False, num_workers=4, pin_memory=True))

# 本地更新类的对象列表
local_update_list = [LocalUpdate(id=idx, dataset=dataset_train, device=device, idxs=dict_users[idx], local_model=ClientModel(inChannel, device=device))
                        for idx in range(num_users)]
...
with trange(num_rounds, colour='red', desc='Round') as t:
    for round in t:
        # selected clients
        idxs_users = np.random.choice(np.arange(
            num_users, dtype=np.int32), int(num_users*frac_users), replace=False)
        ...
        if use_avg:
            ...
            global_weights = aggregate_att(local_weights, global_model.model.state_dict())
            global_ema_weights = aggregate_att(local_ema_weights, global_model_ema.model.state_dict())
        ...
        if verbose:
            log.info('Global Round {:3d}, Train average loss {:.5f}'.format(round+1, loss_avg))
        ...
        summary_writer.add_scalar(
            'Test-Loss/alluser3', mean_ls3, round+1)
        summary_writer.add_scalar(
            'Test-Loss/alluser', mean_ls, round+1)

        loss_test_list.append(mean_ls)
        log.info('For all users, mean of test loss is {:.5f}, std of test loss is {:.5f}'.format(
        ...
        summary_writer.add_scalar(
            'Test-Loss/global', g_m_ls, round+1)
        summary_writer.add_scalar(
            'Test-Loss/global_ema', g_emam_ls, round+1)
